...
import { ORDER_STATUS_CONFIG, formatMoney, formatDateTime } from '../order-utils';
...
export default async function OrderDetailPage({ params }: PageProps) {
  ...
  const { data: order } = await supabase
    .from('orders')
    .select('*, order_items(*), profiles!inner(display_name, email)')
    .eq('id', id)
    .eq('store_id', storeId)
    .single();
  ...
  const profilesRaw = order.profiles as unknown as {
    display_name: string | null;
    email: string | null;
  }[];
  const customer = profilesRaw[0] ?? { display_name: null, email: null };
